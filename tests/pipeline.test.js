...
function run(script, extraArgs = [], { expectFail = false } = {}) {
  // Convert .js → .ts and run with tsx (all .js files were deleted after TS migration)
  const tsScript = script.replace(/\.js$/, '.ts');
  try {
    const out = execFileSync(NODE, ['--import', 'tsx', join(SCRIPTS, tsScript), ...extraArgs], {
      encoding: 'utf8',
...
describe('Sprint 6: preflight-check.js standalone', () => {
  test('S6: preflight-check.ts script file exists and has --help', () => {
    const p = join(SCRIPTS, 'preflight-check.ts');
    assert.ok(existsSync(p), `preflight-check.ts should exist at ${p}`);
...
function runFromRoot(script, extraArgs = [], { expectFail = false } = {}) {
  const jsPath = join(PROJECT_ROOT, script);
  // If the .js file exists (e.g. wizard.js), run it with --import tsx so it can
  // resolve imports to scripts/ (which now only has .ts files after migration).
  // If only .ts exists, convert the extension and use --import tsx.
  const useTsx = true; // Always use tsx — all scripts/ imports are .ts now
  const resolvedPath = existsSync(jsPath) ? jsPath : join(PROJECT_ROOT, script.replace(/\.js$/, '.ts'));
  const args = useTsx ? ['--import', 'tsx', resolvedPath, ...extraArgs] : [resolvedPath, ...extraArgs];
  try {
...
describe('S37: Sprint 15 — FramerExport Cache Tests', () => {
  ...
  test('SP15: callParallel dead fallback updated to 5', async () => {
    ...
    assert.ok(
      src.includes('this.defaultConcurrency ?? 5') || src.includes('defaultConcurrency ?? 5'),
