...
function checkBinding(obj, violations, currentId = 'root') {
  if (typeof obj !== 'object' || obj === null) return;

  // Prüfe auf Element-Struktur
  if (obj.id && obj.settings && obj.styles) {
    const elementId = obj.id;
    const styleIds = Object.keys(obj.styles);
    
    // Hole gebundene Klassen (kann in value sein oder direkt ein Array, je nach V4 Struktur)
    let boundClasses = [];
    if (obj.settings.classes) {
      if (Array.isArray(obj.settings.classes)) {
        boundClasses = obj.settings.classes;
      } else if (obj.settings.classes.value && Array.isArray(obj.settings.classes.value)) {
        boundClasses = obj.settings.classes.value;
      }
    }
    ...
    if (unboundStyles.length > 0) {
      violations.push({
        elementId: elementId,
        widgetType: obj.widgetType || obj.widget_type || obj.elType || obj.el_type || 'unknown',
        definedStyles: styleIds,
        boundClasses: boundClasses,
  ...
  const kids = obj.elements ?? obj.children ?? obj.content ?? [];
  if (Array.isArray(kids)) {
    for (const child of kids) checkBinding(child, violations, obj.id || currentId);
  }
}
