...
const { values: args } = parseArgs({
  options: {
    ...
    // P4-D: Design-Diff Report + Pattern Library
    'design-diff': { type: 'string' },
  },
  strict: false,
});
...
function g6_NoHardcodedHex() {
  ...
  for (const node of nodes) {
    walkValues(node.styles, (key, val, parent, p) => {
      // Skip properly wrapped colors: $$type:"color" wrapping means the hex
      // is an intentional local color value, not a hardcoded-hex bug.
...
// ─────────────────────────────────────────────
// 15. DESIGN_DIFF_SCORE  [P4-D]
// ─────────────────────────────────────────────
//
// Prüft ob ein design-diff Report existiert und der Score ≥ 85 beträgt.
// Der Report wird über --design-diff <path> übergeben.
// Bei fehlendem Report: WARN (nicht FAIL — design-diff ist optional).
//
function g15_DesignDiffScore() {
  const ddPath = args['design-diff'];

  if (!ddPath) {
    return { id: 'DESIGN_DIFF_SCORE', status: 'WARN', severity: 'warning',
      message: 'Kein --design-diff Report angegeben. Design-Fidelity Gate wird nicht geprüft.' };
  }

  if (!fs.existsSync(ddPath)) {
    return { id: 'DESIGN_DIFF_SCORE', status: 'WARN', severity: 'warning',
      message: `Design-Diff Report nicht gefunden: ${ddPath}` };
  }

  try {
    const ddReport = JSON.parse(fs.readFileSync(ddPath, 'utf8'));
    const score = ddReport.meta?.overall_score;
    const maxSeverity = ddReport.meta?.max_severity;

    if (score === undefined || score === null) {
      return { id: 'DESIGN_DIFF_SCORE', status: 'FAIL', severity: 'error',
        message: `Design-Diff Report hat keinen overall_score: ${ddPath}` };
    }

    if (score >= 85) {
      return { id: 'DESIGN_DIFF_SCORE', status: 'PASS',
        message: `Design-Diff Score: ${score}/100 ≥ 85 — Gate PASS` };
    }

    return { id: 'DESIGN_DIFF_SCORE', status: 'FAIL', severity: 'error',
      message: `Design-Diff Score: ${score}/100 < 85 — Design-Fidelity nicht erreicht`,
      details: { score, max_severity: maxSeverity, threshold: 85 } };
  } catch (e) {
    return { id: 'DESIGN_DIFF_SCORE', status: 'FAIL', severity: 'error',
      message: `Design-Diff Report konnte nicht gelesen werden: ${e.message}` };
  }
}

// ─────────────────────────────────────────────
// 16. PATTERN_LIBRARY_CHECK  [P4-D]
// ─────────────────────────────────────────────
//
// Prüft ob FRAMER-VS-ELEMENTOR-PATTERNS.md im Repository-Root existiert.
// Die Pattern Library ist Pflichtlektüre vor jedem Build (AGENTS.md Golden Rule 7).
//
function g16_PatternLibraryCheck() {
  const rootDir = path.resolve(args.tree, '..', '..', '..'); // Heuristik: 3 Ebenen hoch vom V4-Tree
  const possiblePaths = [
    path.resolve(process.cwd(), 'FRAMER-VS-ELEMENTOR-PATTERNS.md'),
    path.resolve(args.tree, '..', '..', '..', '..', 'FRAMER-VS-ELEMENTOR-PATTERNS.md'),
    path.resolve(args.tree, '..', '..', '..', 'FRAMER-VS-ELEMENTOR-PATTERNS.md'),
  ];
  ...
  if (foundPath) {
    ...
    return { id: 'PATTERN_LIBRARY_CHECK', status: 'PASS',
      message: `FRAMER-VS-ELEMENTOR-PATTERNS.md gefunden (${sizeKb} KB) — Pattern Library verfügbar` };
  }

  return { id: 'PATTERN_LIBRARY_CHECK', status: 'WARN', severity: 'warning',
    message: 'FRAMER-VS-ELEMENTOR-PATTERNS.md nicht gefunden. Pattern Library fehlt — Design-Fidelity Risiko.' };
}
...
const guards = [
  ...
  g15_DesignDiffScore(),      // P4-D: Design-Fidelity Gate
  g16_PatternLibraryCheck(),  // P4-D: Pattern Library Verfügbarkeit
];
