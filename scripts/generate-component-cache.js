#!/usr/bin/env node
/**
 * generate-component-cache.js — Phase 10+12: Component Resolution
 *
...
if (!args.tree) {
  ...
  console.error('Usage: node scripts/generate-component-cache.js --tree <v4-tree.json> [--components-dir <dir>] [--token-map <token-mapping.json>] --output <cache.json>');
  process.exit(2);
}
...
const componentsDir = args['components-dir'] || null;
const tokenMapPath  = args['token-map'] || null;
 ...
 *
 * @param {string} xmlContent - Raw Framer XML (from Unframer getNodeXml)
 * @returns {object|null} V4 widget tree or null on failure
 */
function convertXmlToV4(xmlContent) {
  ...
  try {
    ...
    try {
      const v4Tree = JSON.parse(stdout);
      ...
      warn(`Failed to parse convert-xml-to-v4.js output as JSON: ${parseErr.message}`);
      return null;
    }
  } catch (err) {
    warn(`convert-xml-to-v4.js spawn failed: ${err.message}`);
    return null;
  }
}
    ...
    if (fs.existsSync(xmlPath)) {
      ...
      try {
        ...
        if (converted) {
          ...
          resolvedCount++;
          log(`    → Resolved: ${converted.widgetType || converted.type || 'tree'} (${countNodes(converted)} nodes)`);
        } else {
        ...
        warn(`    → Fehler beim Lesen von ${xmlPath}: ${err.message}`);
