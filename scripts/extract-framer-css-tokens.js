#!/usr/bin/env node
/**
 * extract-framer-css-tokens.js — Phase 2: Dual-Source CSS Extraction
 *
 ...
 *
 * Usage:
 *   node scripts/extract-framer-css-tokens.js \
...
const log  = (...m) => { if (args.verbose) process.stderr.write('[css-tokens] ' + m.join(' ') + '\n'); };
...
export async function fetchPageHtml(url) {
  ...
  while ((lm = linkRe.exec(html)) !== null) {
    ...
    try {
      ...
      warn(`External CSS fetch failed: ${cssUrl.slice(0,80)} — ${e.message}`);
    }
  }
...
function extractFontFaces(styleBlocks) {
  const fonts = [];
  const allCss = styleBlocks.join('\n');
...
export function extractBreakpoints(styleBlocks) {
  ...
  while ((m = re.exec(allCss)) !== null) {
    ...
    if (!breakpoints.find(b => b.width === width)) {
      let label = 'custom';
 ...
 *   "/Theme Color/Very Dark Green" als erwarteten Farbwert hat → Match.
 *
 ...
 *
 * @param {Array} cssVars - Extrahierte CSS-Variablen
 * @param {object|null} styleRefs - Unframer-Style-Referenzen ({colors: {path: {expectedColor?}}})
 * @returns {object} { mapped: {path→hex}, unmapped: [tokenNames] }
 */
function mapTokensToStyleRefs(cssVars, styleRefs) {
  const mapped = {};
  const unmapped = [];
  ...
  if (styleRefs?.colors) {
    const refHexMap = new Map(); // hex → stylePath
...
function groupFontsByFamily(fontFaces) {
...
async function main() {
  ...
  if (args.url) {
    htmlContent = await fetchPageHtml(args.url);
    ...
    log(`Read ${htmlContent.length} bytes from ${args.html}`);
  } else {
  ...
  let styleRefs = null;
  if (args['style-refs']) {
    if (fs.existsSync(args['style-refs'])) {
      styleRefs = JSON.parse(fs.readFileSync(args['style-refs'], 'utf8'));
  ...
  if (args.output) {
    ...
    fs.writeFileSync(args.output, outputJson, 'utf8');
