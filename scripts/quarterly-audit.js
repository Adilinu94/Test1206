 ...
 *
 * USAGE:
 *   node scripts/quarterly-audit.js [--output reports/quarterly-{date}.html]
 *   # oder programmatisch:
 ...
 * @param {object} [options.memoryStore] - Optional: { getStats(), decayStale() }
 * @returns {Promise<{html: string, json: object}>}
 */
export async function runQuarterlyAudit({ cacheDir, outputDir = 'reports', memoryStore } = {}) {
  ...
  // 2. Bug-Regression: prüfen ob die Bug-Fix-Dateien existieren
  ...
  // 3. Pipeline-Performance (aus .framer-export-cache/metrics/)
  ...
  // 4. Elementor / WP-Version (ohne MCP — nur Cache lesen)
...
function getPerformanceTrend(cacheDir) {
  ...
  try {
    ...
    const firstDuration = first.total_duration_ms || 0;
    const lastDuration = last.total_duration_ms || 0;
...
function getElementorVersionCache(cacheDir) {
...
function getThemeCache(cacheDir) {
...
function getSecurityAudit(cacheDir) {
