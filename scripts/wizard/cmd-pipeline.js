...
OPTIONS:
  ...
  --design-diff-min-score N   Design-Diff Score-Schwellwert (default: 85). Build blockiert wenn Score < N.
  --no-apply-fixes      Auto-Fix (apply-design-diff-fixes.js --inject) bei Gate-Fail deaktivieren.
...
BEISPIELE (Design-Diff Gate):
  # Default: Gate bei Score < 85 + Auto-Fix mit CSS-Inject
  node wizard.js pipeline --url https://example.framer.app/ --post-id 4943

  # Strikter Score (z. B. 95%) ohne Auto-Fix
  node wizard.js pipeline --url https://example.framer.app/ --post-id 4943 \\
    --design-diff-min-score 95 --no-apply-fixes
`);
}
...
export async function runPipeline({
  ...
  verbose = false,
  siteId = 'default',
  designDiffMinScore = 85,      // Lücke 6: Default-Schwellwert für Design-Diff Gate
  applyFixes = true,             // Lücke 6: Auto-Fix bei Gate-Fail (kann via --no-apply-fixes deaktiviert werden)
}) {
  ...
  let themeEnv = null;
  // Lücke 6: Design-Diff Gate-Konstanten (eindeutig benannt, vermeidet ReferenceError)
  const DESIGN_DIFF_MIN_SCORE = designDiffMinScore;
  const DESIGN_DIFF_APPLY_FIXES = applyFixes;
  ...
  if (v4TreePath && existsSync(v4TreePath)) {
    ...
    // Task: Design-Diff (Pre-Build Gate — gegen Framer-Live-Page)
    // Lücke 6: Gate blockt Builds unter Score 85 (--min-score) und triggert Auto-Fix.
    if (!skipQa && framerUrl) {
      const elementorUrl = postId
        ? `http://rundmund.local/?p=${postId}`
        : null;
      if (elementorUrl) {
        const designDiffPath = path.join(qaDir, 'design-diff.json');
        const designDiffArgs = [
          path.join(pipelineDir, 'design-diff.js'),
          '--framer-url', framerUrl,
          '--elementor-url', elementorUrl,
          '--output', designDiffPath,
          '--timeout', '45000',
          '--min-score', String(DESIGN_DIFF_MIN_SCORE),  // Default 85 — blockt Build bei Score darunter
        ];
        if (verbose) designDiffArgs.push('--verbose');
        postValidationTasks.push({
          command: nodeBin,
          args: designDiffArgs,
          description: 'Design-Diff (CSS Gate)',
          cwd: pipelineDir,
          optional: true,
          outputFile: designDiffPath,
        });
      }
    }
    ...
    const ddResult = postResults.find(r => r.description === 'Design-Diff (CSS Gate)');
    if (!skipQa) {
      if (qaResult?.ok) {
      ...
      // Design-Diff Ergebnis
      if (ddResult) {
        let ddScore = 0;
        try {
          const ddReport = JSON.parse(await fs.readFile(ddResult.outputFile, 'utf8'));
          ddScore = ddReport.meta?.overall_score || 0;
          const ddSeverity = ddReport.meta?.max_severity || 'FAIL';
          const ddPass = ddScore >= DESIGN_DIFF_MIN_SCORE;

          steps.push({
            step: 15,
            name: 'Design-Diff (CSS Gate)',
            status: ddPass ? 'ok' : 'failed',
            score: ddScore,
            detail: `${ddScore}/100, severity=${ddSeverity}, threshold=${DESIGN_DIFF_MIN_SCORE}`,
          });

          if (!ddPass) {
            // Lücke 6: Gate FAIL — Score unter Schwellwert.
            // 1. Auto-Fix per apply-design-diff-fixes.js (Default: an)
            // 2. Mark Pipeline-Step als 'failed' (NICHT nur 'warning')
            // 3. result.summary.results.failed wird hochgezählt → Build gilt als misslungen
            log.warn(`Design-Diff: Score ${ddScore}/100 < ${DESIGN_DIFF_MIN_SCORE} — Design-Fidelity nicht erreicht.`);

            if (DESIGN_DIFF_APPLY_FIXES && postId && !dryRun) {
              try {
                const fixOutputPath = path.join(qaDir, 'apply-fixes.css');
                const fixArgs = [
                  path.join(pipelineDir, 'apply-design-diff-fixes.js'),
                  '--report', ddResult.outputFile,
                  '--output', fixOutputPath,
                  '--inject',
                  '--post-id', String(postId),
                  '--min-severity', 'WARN',
                ];
                const fixResult = await runFile(nodeBin, fixArgs, 'Design-Diff Auto-Fix (CSS Inject)', pipelineDir, fixOutputPath);
                // Guard: Output fehlt wenn runFile erfolgreich war aber Datei nicht geschrieben wurde
                const fixBytes = existsSync(fixOutputPath) ? fs.statSync(fixOutputPath).size : 0;
                steps.push({
                  step: 16,
                  name: 'Design-Diff Auto-Fix (CSS Inject)',
                  status: fixBytes > 0 ? 'ok' : 'warning',
                  detail: `${fixBytes} bytes CSS injected to post ${postId}`,
                });
                log.success(`Design-Diff Auto-Fix: ${fixBytes} bytes CSS → post ${postId}`);
              } catch (fixErr) {
                steps.push({
                  step: 16,
                  name: 'Design-Diff Auto-Fix (CSS Inject)',
                ...
                log.warn(`Design-Diff Auto-Fix fehlgeschlagen: ${fixErr.message}`);
              }
            } else {
              const skipReason = dryRun
                ? 'dry-run'
                : (!DESIGN_DIFF_APPLY_FIXES ? '--no-apply-fixes' : 'no --post-id');
              steps.push({
                step: 16,
                name: 'Design-Diff Auto-Fix (CSS Inject)',
                status: 'skipped',
              ...
              log.info(`Design-Diff Auto-Fix übersprungen (${skipReason}).`);
            }
          } else {
            log.success(`Design-Diff: Score ${ddScore}/100 ≥ ${DESIGN_DIFF_MIN_SCORE} — Gate PASS.`);
            steps.push({
              step: 16,
              name: 'Design-Diff Auto-Fix (CSS Inject)',
              status: 'skipped',
              detail: 'Gate PASS — kein Fix nötig',
            });
          }
        } catch {
          steps.push({ step: 15, name: 'Design-Diff (CSS Gate)', status: 'failed', error: 'No valid report' });
        }
      }
    } else {
      steps.push({ step: 14, name: 'Visual QA + Auto-Fix', status: 'skipped' });
      if (ddResult) steps.push({ step: 15, name: 'Design-Diff (CSS Gate)', status: 'skipped' });
    }
  } else {
    log.warn('Kein V4-Tree gefunden. Überspringe Validation + QA.');
    steps.push({ step: 12, name: 'Pre-Build-Validation', status: 'skipped' });
    steps.push({ step: 14, name: 'Visual QA + Auto-Fix', status: 'skipped' });
    steps.push({ step: 15, name: 'Design-Diff (CSS Gate)', status: 'skipped' });
  }
