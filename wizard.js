  ...
  try {
    ...
    const manifest = {
      ...
      nextSteps: [
        ...
        '2b. cross-validate-sources.js --mcp-json design-system-export.json --export-dir <exportDir> (nach MCP-Export)',
...
if (sub === 'pipeline') {
  ...
  const ddMinScoreIdx = process.argv.indexOf('--design-diff-min-score');
  const framerUrl = urlIdx >= 0 ? process.argv[urlIdx + 1] : null;
  const postId = postIdIdx >= 0 ? process.argv[postIdIdx + 1] : null;
  ...
  // Lücke 6: Design-Diff Gate-Defaults aus CLI-Flags überschreibbar.
  // Default 85 + Auto-Fix an — sonst via Flags änderbar.
  const designDiffMinScore = ddMinScoreIdx >= 0
    ? parseInt(process.argv[ddMinScoreIdx + 1], 10) || 85
    : 85;
  const applyFixes = !process.argv.includes('--no-apply-fixes');
  ...
  await runPipeline({
    framerUrl, postId, exportDir, siteId,
    noCache, skipQa, dryRun, verbose,
    designDiffMinScore, applyFixes,
  });
  process.exit(0);
}
