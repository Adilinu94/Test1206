...
## 7-Schritt Checkliste (in dieser Reihenfolge)

### ✅ Schritt 0a — Framer-URL prüfen [NEU — P4-D]

**Jede Session: Vor Build-Beginn sicherstellen, dass die Framer-Live-URL bekannt ist.**

```
FRAMER_URL: [hier eintragen — z.B. https://meine-seite.framer.app/]
Source: [Framer MCP | manuell aus AGENTS.md | Letzte Session]
```

**Wenn URL vorhanden:**
- Layout-Map generieren: `node scripts/analyze-framer-layout.js --url "$FRAMER_URL"`
- Design-Diff vorbereiten: `--framer-url "$FRAMER_URL"` ans Build-Kommando anhängen

**Wenn keine URL:**
- Build läuft ohne Live-Vergleich (Design-Diff Gate deaktiviert)
- Keine Layout-Prüfung gegen Original möglich
- In SESSION-STATE.md vermerken: `LIVE_URL_MISSING=true`

---

### ✅ Schritt 0b — FRAMER-VS-ELEMENTOR-PATTERNS.md gelesen? [NEU — P4-D]

**Pflicht: Vor jedem Build die Pattern Library konsultieren.**

```
□ FRAMER-VS-ELEMENTOR-PATTERNS.md gelesen (insb. Forbidden Patterns)
□ Layout-Map des aktuellen Projekts verfügbar (aus Schritt 0a)
□ Bekannte Pattern-Konflikte identifiziert (absolute Header, Flex-Rows, Pill-Buttons)
```

**Forbidden Patterns (Quick-Check):**
```
❌ Elementor-Kit-Farben statt Framer-Hex-Werte
❌ Schriftarten raten statt Framer-Fonts
❌ Elementor-Default border-radius akzeptieren
❌ Separate Columns für Button-Gruppen (→ Flex-Container)
❌ Header als normale Section statt position: absolute
```

---

### ✅ Schritt 0g — Framer Live Animation Extraction [NEU — Phase 1 Animation Pipeline]

**Pflicht: Vor jedem Build die Framer-Live-Page auf Animationen analysieren.**
Läuft parallel zu Schritt 0a (Layout-Map). Extrahiert `data-framer-appear-id`
Elemente und generiert einen GSAP-Animation-Plan.
...
**Erwartetes Ergebnis:**
```
✓ N Elemente mit data-framer-appear-id gefunden
✓ M mit erkennbaren Animationen (N-M above-fold, mit Defaults geflaggt)
✓ GSAP ScrollTrigger Snippet generiert
✓ Animation Plan → animation-plan-live.json
```

**Wenn keine Framer-URL:**
- Statischen Export-Extraktor nutzen:
  `node scripts/framer-animation-extractor.js --html FramerExport/index.html --output animation-plan-export.json`
- In SESSION-STATE.md vermerken: `LIVE_ANIMATION_EXTRACTION_SKIPPED=true`

**Wenn Playwright nicht installiert:**
- Script produziert automatisch Dry-Run-Report
- Statischer Export-Extraktor als Fallback
...
[Session-Start]
  │
  ├── 0a. Framer-URL prüfen + Layout-Map generieren    [NEU P4-D]
  ├── 0b. FRAMER-VS-ELEMENTOR-PATTERNS.md gelesen?      [NEU P4-D]
  ├── 0g. Framer Live Animation Extraction              [NEU Phase 1]
