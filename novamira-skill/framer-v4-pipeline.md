...
description: Vollstaendiger Workflow fuer die Konvertierung einer Framer-Seite nach Elementor V4 mit dem framer-v4-pipeline-v2 Repository. Enthaelt alle 18 Schritte, Entscheidungslogik, kritische Invarianten und Fehlerbehandlung. Ability-Namen ausgerichtet auf Novamira AdrianV2 (novamira-adrianv2/*, ohne 'adrians-' Praefix).
version: "0.7.0"
...
1. NIEMALS novamira-adrianv2/batch-build-page fuer Framer-Trees -> elementor-set-content verwenden
2. setup-v4-foundation NIEMALS cachen (GV-IDs + GC-IDs sind session-live, laufen ab)
   novamira-adrianv2/export-design-system DARF 5 Minuten gecacht werden (read-only, sicher)
...
- Pruefe Novamira MCP: novamira-adrianv2/setup-v4-foundation Test-Call
...
> Das ist korrekt — `novamira-adrianv2/export-design-system` ist read-only und ändert sich nicht
> während eines Pipeline-Durchlaufs. Nur `novamira-adrianv2/setup-v4-foundation` darf nie gecacht
...
```
gc-plan.json: MCP-Calls via novamira/execute-php + novamira-adrianv2/add-global-class-variant
...
```
MCP: novamira-adrianv2/setup-v4-foundation { "post_id": POST_ID }
```
...
**Schritt 13** - Layout-Audit (PFLICHT):
```
MCP: novamira-adrianv2/layout-audit { "post_id": POST_ID }
```
...
Bei Fehlern -> novamira-adrianv2/patch-element-styles mit add_style oder settings.
Tipp: element_id stammt aus convert-xml-to-v4.js (Framer nodeId wird 1:1 uebernommen) -> element_ids bleiben stabil ueber Builds hinweg fuer spaetere patch-element-styles Calls.

**Schritt 14** - Visual QA (NICHT ueberspringen!):
```
MCP: novamira-adrianv2/visual-qa { "post_id": POST_ID, "breakpoints": ["desktop","tablet","mobile"] }
```
Prueft: overflow, z-index Konflikte, negative margins, absolute overlap.

**Schritt 15** - Responsive-Audit:
```
MCP: novamira-adrianv2/responsive-audit { "post_id": POST_ID }
```
Bei fehlenden Breakpoints -> novamira-adrianv2/add-global-class-variant (kein Tree-Rebuild noetig):
```
MCP: novamira-adrianv2/add-global-class-variant { "class_id": "gc-xxx", "breakpoint": "mobile", "props": {...} }
```

**Schritt 16** - Variable-Drift-Check (NEU - Fix 5):
```
MCP: novamira-adrianv2/variable-audit { "report": "drift" }
```
Findet e-gv-* Referenzen die nicht mehr im Design-System existieren.
Bei Drift -> novamira-adrianv2/export-design-system -> design-token-extractor.js --existing-tokens.

**Schritt 17** - Style-Patches bei QA-Fehlern:
```
MCP: novamira-adrianv2/patch-element-styles { "post_id": POST_ID, "patches": [
...
```
MCP: novamira-adrianv2/batch-get-content { "post_ids": [ID1, ID2], "mode": "skeleton" }
```
...
| GV-ID Drift | Kit-Update hat IDs verschoben | novamira-adrianv2/export-design-system -> cross-validate |
| GV-ID Drift (schnell) | e-gv-* nicht im Design-System | novamira-adrianv2/variable-audit { report: "drift" } -> Schritt 16 |
| Pass-through nach Build | Zu tiefe Verschachtelung | novamira-adrianv2/layout-audit -> IDs notieren -> patch-element-styles |
| Responsive fehlt | Global Class ohne mobile Variant | novamira-adrianv2/add-global-class-variant (kein Tree-Rebuild) |
...
variables-plan.json       <- MCP-Calls fuer novamira-adrianv2/batch-create-variables (strategy:skip)
design-system-export.json <- novamira-adrianv2/export-design-system Output
...
```bash
npm test               # Regressionstests (aktuelle Anzahl: siehe README.md)
